...
Base64 constexpr encoder and decoder
*/
...
///base64 constexpr class
class Base64 {
public:

    ///Construct Base64 class 
    /**
    @param charset contains 64 characters each for single code.
    @param terminator contain terminator character. Set 0 to disable terminator
     */
    constexpr Base64(const char *charset, char terminator):_terminator(terminator) {
...
inline constexpr auto base64 = Base64{"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/",'='};
inline constexpr auto base64url = Base64{"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-_",'\0'};

///Construct embed BinaryData from inline Base64 string
/**
  usage: constexpr auto bin_data = BinaryData("base64string");

  you can constexpr convert bin_data to basic_string_view<unsigned char>
  you can convert bin_data to string_view (but not constexpr)
*/
...
public:
    ...
    constexpr BinaryData(const char *txt) {
...
template<std::size_t N>
BinaryData(const char (&)[N]) -> BinaryData<N>;
